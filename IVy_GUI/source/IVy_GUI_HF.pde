...
public void handleButtonEvents(GButton button){
  ...
  if(btnStartSweep == button && button.eventType == GButton.CLICKED){
    float startV = float(txfStartV.viewText()); // get start voltage from text field
    float stopV = float(txfStopV.viewText()); // get stop voltage from text field
    int numPoints = round(float(txfNumPoints.viewText())); // get number of data points from text field
...
public void updateGraph(){
  ...
  float maxGraphCurrent = 10.0 * ceil(maxAbsCurrent / 10.0);
  float maxGraphVoltage = ceil(maxAbsVoltage * 10.0) / 10.0;
  
  // figure out tick spacing
  int tickX;
  int tickY;
  float tickXSpacing = (pPlotRight - pPlotCenter)/5.0;
  float tickYSpacing = (pPlotMiddle - pPlotTop)/5.0;
  float currentSpacing = maxGraphCurrent/5.0;
  float voltageSpacing = maxGraphVoltage/5.0;
